# project\.md（Hertz Web 服务专属工程规范）

## 优先级：全局最高，所有 Hertz Web 前端网关/后台管理服务强制生效

**一句话定位**：project\.md = Hertz Web 项目代码怎么写、工程怎么建、规范怎么守
...
5\. 严格禁止：根目录直接存放html、单业务多层嵌套、目录层级混乱；
...
1\. 仅负责：**DB模型 ↔ Hertz HTTP模型** 双向字段映射、默认值补齐；
...
2\. gormL：表模型、Repo仓储、CURD 能力；

3\. where：链式查询构造器，类型安全、杜绝裸SQL；
...
\- biz/handle → logic、convert、dal、redis缓存

\- biz/logic → convert、dal、redis缓存
...
## 五、DAL 层强制规范（与RPC完全统一）

### 5\.1 Where 查询构造器规范

1\. 基类 `gormL.BaseWhere` 统一管理查询条件、SQL构建；

2\. 所有查询必须使用链式 Where 构造，**禁止手写裸SQL**；
